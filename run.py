...
from fast_whisper import Config, load_models, run_baseline, run_speculative, run_experiment, save_results, print_results

def parse_args():
    parser = argparse.ArgumentParser(description="Fast Whisper Transcription")
...
def single_run(args, models, dataset, wer_metric):
    config = Config(top_p=args.top_p, draft_k=args.draft_k, temperature=args.temperature, max_new_tokens=args.max_new_tokens)
    ...
    print(f"\n{'='*60}\nRESULTS\n{'='*60}\n\nBaseline ({mode})\n  Time: {base['avg_time']:.3f}s/sample | WER: {wer_base:.4f}")
    print(f"\nSpeculative (draft_k={args.draft_k})\n  Time: {spec['avg_time']:.3f}s/sample | WER: {wer_spec:.4f}\n\nSpeedup: {speedup:.2f}x\n{'='*60}")
    
    return {
        "config": {"top_p": args.top_p, "draft_k": args.draft_k, "temperature": args.temperature, "max_new_tokens": args.max_new_tokens, "dataset_size": len(dataset)},
        "baseline": {"total_time": round(base["total_time"], 2), "avg_time": round(base["avg_time"], 4), "wer": round(wer_base, 4)},
        "speculative": {"total_time": round(spec["total_time"], 2), "avg_time": round(spec["avg_time"], 4), "wer": round(wer_spec, 4)},
        "speedup": round(speedup, 2),
        "samples": [{"reference": base["references"][i], "baseline": base["predictions"][i], "speculative": spec["predictions"][i]} for i in range(min(3, len(base["references"])))]
    }

def main():
    args = parse_args()
    print(f"Device: cuda/cpu auto-detect\nTarget: {args.target_model}\nDraft:  {args.draft_model}\n\nLoading models...")
    config = Config(target_model=args.target_model, draft_model=args.draft_model)
    ...
    if args.experiment:
        ...
        results = run_experiment(models, dataset, wer_metric, top_p_values, draft_k_values, config)
